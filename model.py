...
class Model(object):
  def __init__(self, MULT_A = 1.0, LAM_REG_WEIGHT = 0):
    ...
    f_conv1 = tf.stop_gradient(tf.floor(h_conv1 * self.MULT_A) * self.MULT_B - self.MULT_B/2.0 - h_conv1) + h_conv1
    reg_conv1 = tf.reduce_sum(tf.abs(h_conv1 - f_conv1 ))
    ...
    f_conv2 = tf.stop_gradient(tf.floor(h_conv2 * self.MULT_A) * self.MULT_B - self.MULT_B/2.0 - h_conv2) + h_conv2
    reg_conv2 = tf.reduce_sum(tf.abs(h_conv2 - f_conv2))
    ...
    f_fc1 = tf.stop_gradient(tf.floor(h_fc1 * self.MULT_A) * self.MULT_B - self.MULT_B/2.0 - h_fc1 ) + h_fc1
    reg_fc1 = tf.reduce_sum(tf.abs(h_fc1 - f_fc1))
